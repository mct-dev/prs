...
const assertInstalledPackage = async (projectDir: string) => {
	...
	const version = await run(["node_modules/.bin/prs", "--version"], projectDir)
	assert(version.stdout.trim() === rootPackageJson.version, `Expected prs --version to print ${rootPackageJson.version}, got ${JSON.stringify(version.stdout.trim())}`)
}

const assertCacheServiceOpens = async () => {
	const dir = await mkdtemp(join(tmpdir(), "prs-cache-smoke-"))
	try {
...
const tempRoot = await mkdtemp(join(tmpdir(), "prs-package-smoke-"))
try {
