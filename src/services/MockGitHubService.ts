...
export const MockGitHubService = {
	layer: (options: MockOptions) => {
		...
		const comments = (repository: string, number: number): readonly PullRequestComment[] => [
			...
			// Richer generic shapes (bot, HTML, file-level, outdated, resolved)
			// anchored to the mock diff so inline threads and badges have content.
			{
				_tag: "comment",
				id: `mock-comment:${repository}:${number}:bot`,
				author: "coverage-bot",
				authorIsBot: true,
				body: [
					"## Coverage report",
					"",
					"<details><summary>2 files changed coverage</summary>",
					"",
					"| File | Before | After |",
					"| --- | --- | --- |",
					"| `src/mockDiff.ts` | 80% | 92% |",
					"| `src/App.tsx` | 64% | 64% |",
					"",
					"</details>",
					"",
					'<img src="https://example.com/badge.svg" alt="coverage badge">',
				].join("\n"),
			...
			{
				_tag: "review-comment",
				id: `mock-review:${repository}:${number}:diff-1`,
				author: "mock-reviewer",
				body: "Could `newOne` get a **clearer name**? Something like:\n\n```ts\nconst initialCount = 1\n```",
				createdAt: new Date(Date.now() - 1_700_000),
				url: null,
				path: "src/mockDiff.ts",
				line: 2,
				side: "RIGHT",
				inReplyTo: null,
			},
			{
				_tag: "review-comment",
				id: `mock-review:${repository}:${number}:diff-2`,
				author: "mock-author",
				body: "Good call, renamed in the next commit.",
				createdAt: new Date(Date.now() - 1_600_000),
				editedAt: new Date(Date.now() - 1_500_000),
				url: null,
				path: "src/mockDiff.ts",
				line: 2,
				side: "RIGHT",
				inReplyTo: `mock-review:${repository}:${number}:diff-1`,
			},
			{
				_tag: "review-comment",
				id: `mock-review:${repository}:${number}:diff-bot`,
				author: "lint-bot",
				authorIsBot: true,
				body: "<details><summary>1 warning</summary>\n\n- `newTwo` is declared but never used.\n</details>",
				createdAt: new Date(Date.now() - 1_400_000),
				url: null,
				path: "src/mockDiff.ts",
				line: 4,
				side: "RIGHT",
			...
			{
				_tag: "review-comment",
				id: `mock-review:${repository}:${number}:diff-file`,
				author: "another-reviewer",
				body: "Whole-file note: consider splitting this module.",
				createdAt: new Date(Date.now() - 1_300_000),
				url: null,
				path: "src/mockDiff.ts",
				line: 0,
				side: "RIGHT",
			...
			{
				_tag: "review-comment",
				id: `mock-review:${repository}:${number}:diff-outdated`,
				author: "another-reviewer",
				body: "This line moved in a later commit.",
				createdAt: new Date(Date.now() - 1_100_000),
				url: null,
				path: "src/mockDiff.ts",
				line: 9,
				side: "LEFT",
					...
					? [
							{
								...
								...(comment.authorIsBot ? { authorIsBot: true } : {}),
								...(comment.editedAt ? { editedAt: comment.editedAt } : {}),
								...(comment.subjectType ? { subjectType: comment.subjectType } : {}),
