...
// compile, then fetch each section (4 at a time). Each section publishes its
	...
	Effect.gen(function* () {
		...
		yield* Effect.forEach(
			...
			(team) => {
				...
				return adapter.teamMembers(org, slug).pipe(
		...
		yield* Effect.forEach(
			...
			(section, index) =>
				Effect.gen(function* () {
					if (section.error) return
					const result = yield* Effect.forEach(section.queries, (query) => adapter.search(query, section.limit), { concurrency: SECTION_CONCURRENCY }).pipe(
